...
class GstRecordingMedia(GstMedia):
    ...
    def _init_pipe(self, image):
        desc = "appsrc is-live=true do-timestamp=true name=src format=time ! video/x-raw,width=%s,height=%s,framerate=30/1,pixel-aspect-ratio=1/1,format=%s ! tiovxcolorconvert ! avenc_mpeg4 ! mpegtsmux ! queue ! filesink async=false qos=false location=%s" % (
