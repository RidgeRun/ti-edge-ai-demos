...
class GstRecordingMedia(GstMedia):
    ...
    def _init_pipe(self, image):
        desc = "appsrc do-timestamp=true format=time name=src ! video/x-raw,width=%d,height=%d,format=%s,framerate=30/1 ! tiovxcolorconvert ! avenc_mpeg4 ! mpegtsmux ! filesink location=%s" % (
